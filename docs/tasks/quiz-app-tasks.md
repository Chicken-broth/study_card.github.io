...
- **テスト要件**:
  ...
  - [x] UIからリセットボタンを押すと、学習履歴がリセットされ、関連する表示（例: 未回答フィルター）が正しく更新されることを確認するE2Eテスト。
- **完了条件**:
  - [x] UIから学習履歴をリセットできる。

#### TASK-210: 不正解問題フィルター機能の実装
...
- **依存タスク**: TASK-207
- **実装詳細**:
  - `client/app/src/core/filter.gleam` の `Filter` 型に `is_incorrect_only: Bool` フィールドを追加する。
  - `quiz_home.gleam` の `Model` がこの新しいフィルター状態を持つようにする。
  - `view` に「不正解の問題のみ」チェックボックスを追加する。
  - `update` にチェックボックス操作を処理する `Msg` を追加する。
  - `quiz_home.gleam` にフィルタリングロジックを実装する。`quiz_results` を参照し、最新の学習履歴が「不正解」である問題のみを抽出する。
- **テスト要件**:
  - [ ] フィルターが有効な場合に、最新の履歴が「不正解」の問題のみが抽出されることを確認する単体テスト。
  - [ ] UIからフィルターを操作し、選択される問題数が正しく変わることを確認するE2Eテスト。
- **完了条件**:
  - [ ] UIから「不正解の問題のみ」フィルターを有効にすると、該当する問題のみがクイズ対象となる。
  - [ ] フィルターを無効にすると、すべての問題がクイズ対象に戻る。

### フェーズ5: Firestore移行

#### TASK-211: 組み合わせ問題の採点ロジック変更

- [x] **タスク完了**
- **タスクタイプ**: REFACTOR
- **要件リンク**: REQ-003-1, ADR-0009
- **依存タスク**: TASK-202
- **実装詳細**:
  - `association_question.gleam` のロジックを修正。
  - 一度でも不正解のペアを選択した場合、その問題は「不正解」として判定されるように変更。
  - `Model` に `has_made_mistake` フラグを追加して判定を管理。
- **テスト要件**:
  - [x] 既存のテストがすべてパスすること。
- **完了条件**:
  - [x] 組み合わせ問題で一度でも間違えると、問題全体が不正解として扱われる。

#### TASK-212: 組み合わせ問題に回答状態を表示
...
- **依存タスク**: TASK-211
- **実装詳細**:
  - `association_question.gleam` の `view` 関数を修正。
  - 現在の回答状態（正解/不正解/未回答）を画面に表示する `view_answer_status` ヘルパーを追加。
- **UI/UX要件**:
  - [x] ユーザーが現在の回答状態を即座に認識できる。
- **テスト要件**:
  - [x] 表示が正しく切り替わることを目視で確認。
- **完了条件**:
  - [x] 組み合わせ問題の画面に、現在の回答状態がテキストで表示される。
